...
window.GRR_Shared = {
  ...
  exportActorReactions: (actor) => {
    const reactions = GRR_Shared.getActorReactions(actor);
    const data = {
      actorName: actor.name,
      moduleVersion: game.modules.get(GRR_Shared.MODULE_ID)?.version ?? "?",
      skills: reactions.skills,
      weapons: reactions.weapons,
      universal: reactions.universal,
    };
    const blob = new Blob([JSON.stringify(data, null, 2)], { type: "application/json" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `grr-${actor.name.replace(/[<>:"/\\|?*]/g, '_')}.json`;
    a.click();
    URL.revokeObjectURL(url);
  },

  importActorReactions: async (actor, data) => {
    const mid = GRR_Shared.MODULE_ID;
    const target = GRR_Shared.resolveActor(actor);
    if (data.skills && Object.keys(data.skills).length > 0)
      await target.setFlag(mid, "skills", data.skills);
    if (data.weapons && Object.keys(data.weapons).length > 0)
      await target.setFlag(mid, "weapons", data.weapons);
    if (data.universal && Object.keys(data.universal).length > 0)
      await target.setFlag(mid, "universal", data.universal);
  },
